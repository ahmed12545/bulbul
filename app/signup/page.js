      ...
      <div className="auth-card">
          ...
          {password.length > 0 && (
            <div className="auth-password-strength">
              <div className="auth-strength-bar">
                <div className={`auth-strength-fill ${isLongEnough ? "strong" : "weak"}`}></div>
              </div>
              <div className={`auth-strength-text ${isLongEnough ? "strong" : "weak"}`}>
                {isLongEnough ? (
                  <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="14" height="14"><polyline points="20 6 9 17 4 12" /></svg> Strong enough</>
                ) : (
                  <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="14" height="14"><circle cx="12" cy="12" r="10" /><line x1="15" y1="9" x2="9" y2="15" /><line x1="9" y1="9" x2="15" y2="15" /></svg> At least 8 characters</>
                )}
              </div>
            </div>
          )}
        ...
        <div className="auth-form-group">
          ...
          <div className="auth-input-wrap">
            ...
            />
            <button className="auth-eye-btn" onClick={() => setShowConfirm(!showConfirm)} type="button">
              {showConfirm ? (
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24" /><line x1="1" y1="1" x2="23" y2="23" /></svg>
              ) : (
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>
          ...
          {confirmPassword.length > 0 && (
            <div className={`auth-match-text ${passwordsMatch ? "match" : "mismatch"}`}>
              {passwordsMatch ? (
                <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="14" height="14"><polyline points="20 6 9 17 4 12" /></svg> Passwords match</>
              ) : (
                <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="14" height="14"><circle cx="12" cy="12" r="10" /><line x1="15" y1="9" x2="9" y2="15" /><line x1="9" y1="9" x2="15" y2="15" /></svg> Passwords don&apos;t match</>
              )}
            </div>
          )}
        </div>

        <button className="auth-btn-primary" style={{ marginTop: "4px" }}>Sign up</button>

        <p className="auth-switch">Already have an account? <Link href="/login">Log in</Link></p>
        <p className="auth-footer-note">By signing up, you agree to our <a href="#">Terms</a> and <a href="#">Privacy Policy</a></p>
      </div>
    </div>
  );
}
